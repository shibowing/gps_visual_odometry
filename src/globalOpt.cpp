...
{  
    ...
    //step2:  prediction: global_gps_pose= transform_between_gps_and_vio* vio_pose
    Eigen::Quaterniond globalQ;
    globalQ = WGPS_T_WVIO.block<3, 3>(0, 0) * OdomQ;
    Eigen::Vector3d globalP = WGPS_T_WVIO.block<3, 3>(0, 0) * OdomP + WGPS_T_WVIO.block<3, 1>(0, 3);
    vector<double> globalPose{globalP.x(), globalP.y(), globalP.z(),
                              globalQ.w(), globalQ.x(), globalQ.y(), globalQ.z()};
    //step3:save globalPose
...
// begin ceres optimize process (fuse gps and vio odometry)
void GlobalOptimization::optimize()
{
    while(true)
    {
        if(newGPS)
        {
            ...
            //note: this is QuaternionParameterization not eigen one. Therefore, the quaterion order is w, x, y, z.
            ...
            //step: feed optimized pose as ceres parameter, which are needed to be optimized
            ...
            // add optimize constraints between  gps frame
            for (iterVIO = localPoseMap.begin(); iterVIO != localPoseMap.end(); iterVIO++, i++)
            {
                // step 3: add vio factor into optimization
                iterVIONext = iterVIO;
                iterVIONext++;
                if(iterVIONext != localPoseMap.end())
                {
                    ...
                    // step 4: add T_ji constraints from vio
                    ...
                    //rotation amd translation difference between i and j frame
                    Eigen::Quaterniond iQj;
                    iQj = iTj.block<3, 3>(0, 0);
                    Eigen::Vector3d iPj = iTj.block<3, 1>(0, 3);
                    
                    // add relative residule from sequential frames (including convariance 0.1 for transtaltion and 0.01 for rotation)
            ...
            for (int i = 0; i < length; i++, iter++)
            {
            	...
            	if(i == length - 1)
            	{
            	    ...
            	    WGPS_T_WVIO = WGPS_T_body * WVIO_T_body.inverse();  //To calculate the relative transform
            	}
            }
            updateGlobalPath();
